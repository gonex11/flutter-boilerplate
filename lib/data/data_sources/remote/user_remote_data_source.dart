...
import 'package:flutter_boilerplate/core/services/remote/responses/base_object_response.dart';
  ...
  Future<Either<Failure, UserModel>> getUserById(int id) async {
    try {
      final response = await _apiService.get('http://10.0.2.2:3000/users/$id',
          authorized: false);
      final result = BaseObjectResponse.fromJson(
        response.data,
        (json) => UserModel.fromJson(response.data["data"]),
      );
      return Right(result.data);
    } on ApiException catch (e) {
      return Left(ServerFailure(e.error));
